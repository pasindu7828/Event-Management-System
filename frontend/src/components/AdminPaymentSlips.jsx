...
const AdminPaymentSlips = () => {
  ...
  const filtered = allSlips.filter((s) => {
    ...
    if (searchText.trim()) {
      ...
      const name = `${s.studentId?.firstName || ''} ${s.studentId?.lastName || ''}`.toLowerCase();
                  ...
                  {filtered.map((slip) => (
                    <tr key={slip._id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-4">
                        ...
                        <p className="text-xs text-slate-400">{slip.studentId?.email}</p>
                        {slip.studentId?.studentId && (
                          <p className="text-xs text-slate-400">ID: {slip.studentId.studentId}</p>
                        )}
                      </td>
                      <td className="px-5 py-4">
          ...
          <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-6 py-4">
              <div>
                ...
                <p className="text-xs text-slate-400">Submitted by {selectedSlip.studentId?.firstName} {selectedSlip.studentId?.lastName}</p>
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><p className="font-bold text-slate-500 text-[11px] uppercase tracking-wider">Name</p><p className="font-black text-slate-800">{selectedSlip.studentId?.firstName} {selectedSlip.studentId?.lastName}</p></div>
                  <div><p className="font-bold text-slate-500 text-[11px] uppercase tracking-wider">Email</p><p className="font-black text-slate-800">{selectedSlip.studentId?.email}</p></div>
                  {selectedSlip.studentId?.studentId && (
                    <div><p className="font-bold text-slate-500 text-[11px] uppercase tracking-wider">Student ID</p><p className="font-black text-slate-800">{selectedSlip.studentId.studentId}</p></div>
                  )}
                  {selectedSlip.studentId?.faculty && (
                    <div><p className="font-bold text-slate-500 text-[11px] uppercase tracking-wider">Faculty</p><p className="font-black text-slate-800">{selectedSlip.studentId.faculty}</p></div>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 px-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <div className="mt-3 rounded-xl bg-slate-50 border border-slate-100 px-4 py-3 text-sm text-slate-600">
              <p><span className="font-bold">Student:</span> {actionModal.slip.studentId?.firstName} {actionModal.slip.studentId?.lastName}</p>
