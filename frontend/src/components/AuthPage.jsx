...
//                     <form className="space-y-10 max-w-md w-full" onSubmit={handleLogin}>
...
//                                     <input type="email" placeholder="name@company.com" className="w-full bg-slate-50 border-2 border-slate-100 py-4 px-6 outline-none focus:border-blue-500/50 focus:bg-white focus:ring-4 focus:ring-blue-100 transition-all text-base text-slate-700 placeholder:text-slate-300 font-medium" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} />
...
//                                 </div>
//                                 <div className="relative overflow-hidden rounded-2xl">
//                                     <input type={showPassword ? "text" : "password"} placeholder="••••••••" className="w-full bg-slate-50 border-2 border-slate-100 py-4 px-6 outline-none focus:border-blue-500/50 focus:bg-white focus:ring-4 focus:ring-blue-100 transition-all text-base text-slate-700 placeholder:text-slate-300 font-bold tracking-[0.2em] pr-14" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} />
...
//                 <form className="w-full space-y-4 max-w-sm" onSubmit={handleRegister}>
//                     <input type="text" placeholder="First Name" className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerFirstName} onChange={(e) => setRegisterFirstName(e.target.value)} />
//                     <input type="text" placeholder="Last Name" className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerLastName} onChange={(e) => setRegisterLastName(e.target.value)} />
//                     <input type="text" placeholder="Phone" className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerPhone} onChange={(e) => setRegisterPhone(e.target.value)} />
//                     <input type="email" placeholder="Email Address" className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerEmail} onChange={(e) => setRegisterEmail(e.target.value)} />
//                     <input type="password" placeholder="Password" className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerPassword} onChange={(e) => setRegisterPassword(e.target.value)} />
//                     <input type="password" placeholder="Confirm Password" className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerConfirmPassword} onChange={(e) => setRegisterConfirmPassword(e.target.value)} />
//                     <input type="text" placeholder="Student ID" className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerStudentId} onChange={(e) => setRegisterStudentId(e.target.value)} />
//                     <select className="w-full bg-[#f4f8f7] border-none py-4 px-6 rounded-[20px] outline-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-600/10 transition-all" value={registerRole} onChange={(e) => setRegisterRole(e.target.value)}>
...
//                         <button type="submit" className="px-16 py-4 bg-blue-600 text-white font-bold uppercase tracking-widest text-xs rounded-full shadow-xl shadow-blue-600/20 hover:bg-blue-700 hover:scale-105 transition-all" disabled={loading}>{loading ? 'Signing up...' : 'Sign Up'}</button>
//                     </div>
//                 </form>
//             </div>
//         </div>
...
//     return (
//         <>
//             {isLogin ? <LoginView {...loginViewProps} /> : <RegisterView {...registerViewProps} />}
...
const LoginView = ({
    ...
    <div className="min-h-screen w-full bg-[#0a0a1a] flex items-center justify-center font-sans overflow-hidden relative selection:bg-blue-100 selection:text-blue-900 animate-fade-in">
        ...
        {/* 3. LAYER: ORGANIC AMOEBA WHITE BLOB - Belissa style */}
        <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
            <svg
                className="absolute animate-blob-stable"
                style={{ left: '-8%', top: '-5%', width: '62%', height: '110%' }}
                viewBox="0 0 600 700"
            ...
            >
                <defs>
                    <filter id="blob-shadow" x="-20%" y="-20%" width="140%" height="140%">
                        <feDropShadow dx="0" dy="30" stdDeviation="40" floodColor="rgba(0,0,0,0.3)" />
                    </filter>
                </defs>
                <path
                    d="M80,60 C180,-20 420,-10 520,80 C620,170 580,300 540,400 C500,500 460,640 320,670 C180,700 60,600 30,480 C0,360 -20,140 80,60 Z"
                    fill="white"
                    fillOpacity="0.97"
...
const AuthPage = ({ onHome: onHomeProp, initialMode = 'login' }) => {
    ...
    return (
        ...
        @keyframes blobStable {
            ...
            33%       { transform: translate(6px, -8px) rotate(0.8deg) scale(1.005); }
            66%       { transform: translate(-4px, 4px) rotate(-0.5deg) scale(0.997); }
        }
        @keyframes formReveal { from { opacity: 0; transform: translateX(-40px); } to { opacity: 1; transform: translateX(0); } }
        @keyframes floatParticle { 0% { transform: translateY(0) translateX(0); opacity: 0; } 20% { opacity: 0.8; } 80% { opacity: 0.8; } 100% { transform: translateY(-100vh) translateX(50px); opacity: 0; } }
        @keyframes floatSlow { 0%, 100% { transform: translateY(0) rotate(0deg); } 50% { transform: translateY(-20px) rotate(2deg); } }
